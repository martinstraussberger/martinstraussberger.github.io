...
function createBlogPostNavigation(options = {}) {
    ...
    const breadcrumbs = [
        {
            url: NAVIGATION_CONFIG.homeUrl,
            label: NAVIGATION_CONFIG.homeLabel,
        },
        {
            url: NAVIGATION_CONFIG.blogUrl,
            label: NAVIGATION_CONFIG.blogLabel,
        },
        {
            url: '#',
            label: 'Article',
...
function createProjectNavigation(options = {}) {
    ...
    const breadcrumbs = [
        ...
        {
            url: NAVIGATION_CONFIG.projectsUrl,
            label: NAVIGATION_CONFIG.projectsLabel,
        },
        {
            url: '#',
            label: 'Project',
...
/**
 * Waits for required services to be available
 * @param {Array<string>} serviceNames - Array of service names to wait for (e.g., ['BlogService', 'CardRenderer'])
 * @param {Object} options - Configuration options
 * @param {number} options.maxAttempts - Maximum number of attempts (default: 50)
 * @param {number} options.delay - Delay between attempts in ms (default: 10)
 * @param {string} options.context - Context for error messages (default: 'Component')
 * @returns {Promise<boolean>} - Returns true if all services are available, false otherwise
 */
async function waitForServices(
    ...
    while (attempts < maxAttempts) {
        const allServicesAvailable = serviceNames.every(serviceName => window[serviceName]);
    ...
    console.error(`${context}: Required services failed to initialize: ${serviceNames.join(', ')}`);
    return false;
}
